...
$('.add-cart-large').each(function (i, el) {
	$(el).click(function () {
		...
		let price = parseFloat($(this).parent().find('.product_price').text().replace('$', ''));
		
		// Add the price to the total price of the cart
		totalPrice += price;
		
		// Update the HTML to display the total price of the cart
		$('.total-price').text('$' + totalPrice.toFixed(2));
		document.getElementById("total-price").innerHTML = "$" + totalPrice.toFixed(2);
		console.log(totalPrice);
		console.log(product_price);
		// Rest of the code for adding the product to the cart
		// ...
	});
});
