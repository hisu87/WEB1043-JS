...
// Tạo biến để lưu tổng tiền của giỏ hàng
var total = 0;

// Lặp qua từng mục trong giỏ hàng
$("#cart .cart-item").each(function() {
  // Lấy giá tiền của sản phẩm từ phần tử strong trong mục
  var price = parseFloat($(this).find("product_price").text().replace("$", ""));
  console.log(price)
  // Cộng giá tiền vào tổng tiền
  total += price;
});

// Hiển thị tổng tiền trong giỏ hàng
console.log("Tổng tiền: $" + total.toFixed(2));
document.getElementById("total").innerHTML = "Tổng tiền: $" + total.toFixed(2);